...
def _is_success(
    ...
    *,
    ...
    if max_steps_per_task is None:
        return True
    return int(item.get("steps", 0)) <= max_steps_per_task
    ...
    *,
    ...
    for round_id in sorted(grouped):
        ...
        successes = sum(
            ...
            if _is_success(item, max_steps_per_task=max_steps_per_task)
        )
...
def _overall_summary(
    ...
    *,
    ...
    successful_tasks = sum(
        int(item.get("successful_tasks", 0)) for item in round_summaries
    )
    return {
        "method": label,
        "tasks": tasks,
        "successful_tasks": successful_tasks,
        "success_rate": successful_tasks / tasks if tasks else 0.0,
...
def _memory_timeline_plot(
    *,
    ...
    ax.set_title("Memory Size Timeline Across Task Attempts")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Count successful attempts above this action count as failures.",
    )
    ...
    left_rounds = _group_by_round(
        left_results,
        max_steps_per_task=args.max_steps_per_task,
    )
    right_rounds = _group_by_round(
        right_results,
        max_steps_per_task=args.max_steps_per_task,
    )
