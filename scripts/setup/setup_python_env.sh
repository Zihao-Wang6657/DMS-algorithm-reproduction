...
PYPI_INDEX_URL="${DMS_PYPI_INDEX_URL:-https://pypi.org/simple}"

if [[ ! -d "$ENV_PREFIX/conda-meta" ]]; then
  conda create -y -p "$ENV_PREFIX" python=3.10 sqlite=3.45.3
fi

conda activate "$ENV_PREFIX"
conda install -y sqlite=3.45.3
python -s -m pip install --upgrade pip setuptools wheel
python -s -m pip install \
  --index-url https://download.pytorch.org/whl/cpu \
  --no-deps \
  torch==2.7.1 torchvision==0.22.1
python -s -m pip install --index-url "$PYPI_INDEX_URL" -r "$REQ_FILE"
